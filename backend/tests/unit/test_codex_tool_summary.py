...
@pytest.mark.parametrize("tool_name", ["material_search"])
def test_material_search_summary_keeps_names_and_counts_in_reauthorized_evidence_only(tool_name) -> None:
    payload = {
        "results": [{"name": "견적.md", "excerpt": SECRET}, {"name": "견적.md", "excerpt": SECRET}],
        "searched_materials": 2,
        "unavailable_materials": [{"name": "scan.png"}],
    }
    summary = _summarize_tool_result({"structured_content": payload}, tool_name=tool_name)
    assert summary == "결과 수신 (자료 내용은 근거 카드에만 표시)"
    assert "한빛상사" not in summary


@pytest.mark.parametrize("tool_name", ["material_search"])
def test_material_search_non_json_text_payload_is_never_echoed(tool_name) -> None:
    raw = {"content": [{"type": "text", "text": f"excerpt: {SECRET}"}]}
    summary = _summarize_tool_result(raw, tool_name=tool_name)
