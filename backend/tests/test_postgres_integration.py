...
from ax.runtime import InvalidDecision, LocalDemoToolDispatcher, SqlAlchemyUnitOfWork, WorkflowRunStarter
...
def test_postgres_serializes_competing_human_decisions_before_effect_commit(
    ...
    with SqlAlchemyUnitOfWork(session_factory) as uow:
        ...
        started = WorkflowRunStarter(uow.workflows).start("daily-report", seeded_principal("mina"), {})
    ...
    def decide(decision: str) -> str:
        try:
            with SqlAlchemyUnitOfWork(session_factory) as uow:
                assert uow.workflows is not None
                result = WorkflowRunStarter(uow.workflows).decide(
