...
def create_app(
    ...
    *,
    ...
    # 라우트의 «존재»와 «누가 부를 수 있나»는 다른 질문이다. 예전에는 한 `if` 가 둘을 겸해서,
    # PRODUCTION 이면 API·WS 가 **하나도 등록되지 않았다**(`/health` 만 남았다). 그래서 둘을 가른다 —
    # **등록은 프로파일과 무관**하고, **권한은 종전대로** `http_auth` 의 세션 판정이 정한다.
    ...
    if settings.developer_auth_enabled:
        ...
        # `http_auth.current_principal` 이 이 속성이 없으면 세션만 보고 401 을 낸다.
