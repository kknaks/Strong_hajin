"""scenario CLI — 들어와 있는 조직 위에 예제 업무를 만든다.
    ...
    DATABASE_URL=... uv run python -m ax_workspace.entrypoints.scenario ~/scax-datasets/thesc/scenario.yaml

조직은 정본이고 이것은 아니다. 여기서 만드는 모든 것은 제품의 정식 command를 그 사람으로서 지나가므로 actor·
이력·권한이 진짜다. 지울 수 있는 로컬 demo 데이터베이스에만 쓴다.

계획 파일은 실제 사람 key와 고객사 이름을 가리키므로 조직 dataset과 같이 저장소 밖에 둔다. 저장소는 계약과
loader만 갖는다.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("plan", type=Path, help="저장소 밖의 계획 파일")
    ...
    if plan_path.is_relative_to(repository):
        ...
        print(f"계획 파일은 저장소 밖에 두어야 합니다: {plan_path}", file=sys.stderr)
        return 2
