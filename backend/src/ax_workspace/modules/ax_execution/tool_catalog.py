...
_DEFINITIONS = (
    ...
    ToolDefinition("work_request_inbox", "받은 업무와 참조 요청 조회", "Read received work requests: category=work for pending or negotiating assignee requests, category=reference for CC requests this member has not marked read. Marking a reference read is a human action and has no tool; read references stay available through work_request_list and work_request_get. CC can read request state, results and history but cannot accept or reject. Task references are not inbox entries. Use work_request_get for detail and rounds.", "work_request_inbox", any_capabilities=("work_request.decide", "work_request.read")),
