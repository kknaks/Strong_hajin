...
class PypdfTextExtractor:
    """UTF-8 text/Markdown and text-based PDF only. Failures are reason codes, never raw parser messages."""

    def extract(self, *, name: str, content_type: str, data: bytes) -> ExtractionOutcome:
        extractor = classify(name, content_type)
        if extractor is None:
            return ExtractionOutcome(status="unsupported", extractor=None, failure_reason="unsupported_format")
        if extractor in {"text", "markdown"}:
            ...
            return ExtractionOutcome(status="completed", extractor=extractor, chunks=tuple(chunks), char_count=min(len(decoded), MAX_TEXT_CHARS))
...
class SqlAlchemyMaterialExtractionRepository:
    ...
    def request(self, attachment: AttachmentRecord) -> MaterialExtractionRecord:
        """One extraction per attachment version (integrity hash); requesting again returns the existing row."""
        existing = self._session.scalar(
            select(MaterialExtractionRecord).where(
                MaterialExtractionRecord.attachment_id == attachment.id, MaterialExtractionRecord.integrity_ref == attachment.integrity_ref
            )
        )
    ...
    def for_attachments(self, attachment_ids: list[UUID]) -> dict[UUID, MaterialExtractionRecord]:
        ...
        rows = self._session.scalars(
            select(MaterialExtractionRecord).where(MaterialExtractionRecord.attachment_id.in_(attachment_ids)).order_by(MaterialExtractionRecord.requested_at)
        ).all()
    ...
    def complete(self, extraction: MaterialExtractionRecord, outcome: ExtractionOutcome) -> None:
        # Re-processing the same version replaces its chunks instead of appending duplicates.
