...
class CodexCliProviderAdapter:
    ...
    RELATIONSHIP_POLICY = (
        ...
        "- `내 업무`와 `조직의 업무`는 다른 질문이다. 그 사람이 해야 할 일을 물으면 `task_list`를 그대로 부르고,"
        " 팀이나 프로젝트 전체를 물을 때만 `mine=false`로 넓힌다. 읽을 수 있다는 것이 그 사람의 일이라는 뜻은"
        " 아니므로, 넓혀서 받은 목록으로 `내 업무`를 답하지 않는다.\n"
