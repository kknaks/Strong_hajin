...
export default function App() {
  ...
  const start = async (workflow: Workflow) => {
    ...
    try {
      ...
      await api<Run>(`/api/runs/${workflow.workflow_id}`, persona, { method: "POST", body: JSON.stringify({ input }) });
      await refresh();
    } catch (caught) {
  ...
  const dailyReportWorkflow = workflows.find((workflow) => workflow.workflow_id === "daily-report");
    ...
    <section className="canvas">
      ...
      {surface === "today" && <>
        <div className="dashboard-columns">
          <section className="surface-card requests"><div className="card-title"><h2>오늘 나에게 요청된 업무</h2><button onClick={() => setSurface("work")}>전체보기</button></div><div className="table-head"><span>내용</span><span>상태</span><span>요청자</span><span>판단</span></div>{inbox.length === 0 ? <p className="empty-row">지금 처리할 판단이 없습니다.</p> : inbox.map((item) => <article className="request-row" key={`${item.run_id}:${item.node_id}`}><b>{item.label}</b><span className="status waiting_for_decision">승인대기</span><small>업무 요청</small><div className="row-actions">{item.node_id === "choose-assignment" && <select value={assigneeId} onChange={(event) => setAssigneeId(event.target.value)}>{assignmentCandidates.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.display_name}</option>)}</select>}<button className="outline" disabled={busy !== null} onClick={() => void decide(item, "reject")}>검토하기</button><button className="primary" disabled={busy !== null || (item.node_id === "choose-assignment" && assignmentCandidates.length === 0)} onClick={() => void decide(item, "accept")}>{busy === `${item.run_id}:${item.node_id}` ? "저장 중" : "승인"}</button></div></article>)}</section>
        ...
        <section className="surface-card schedule"><div className="card-title"><h2>오늘의 업무 안내</h2><button onClick={() => setSurface("work")}>내 업무 보기</button></div><p className="empty-row">업무·요청·판단은 각각의 기능 화면에서 관리합니다.</p></section>
      </>}
      {surface === "work" && <section className="surface-card workflow-surface"><div className="card-title"><div><p className="kicker">MY WORK</p><h2>내 업무</h2></div><button onClick={() => void refresh()}>새로고침</button></div><div className="task-create"><input value={taskTitle} placeholder="직접 시작할 업무 제목" onChange={(event) => setTaskTitle(event.target.value)} /><button className="primary" disabled={busy !== null} onClick={() => void createTask()}>{busy === "create-task" ? "추가 중" : "업무 추가"}</button></div></section>}
      {surface === "report" && <section className="surface-card workflow-surface"><div className="card-title"><div><p className="kicker">REPORTS</p><h2>개인 일일보고</h2><p>오늘의 업무 기록을 초안으로 만들고, 확인한 내용만 제출합니다.</p></div></div>{dailyReportWorkflow && <div className="task-create"><span className="version">오늘</span><button className="primary" disabled={busy !== null} onClick={() => void start(dailyReportWorkflow)}>{busy === dailyReportWorkflow.workflow_id ? "초안 생성 중" : "보고 초안 만들기"}</button></div>}</section>}
      {surface !== "today" && surface !== "work" && surface !== "report" && <section className="surface-card workflow-surface"><div className="card-title"><div><p className="kicker">{surface.toUpperCase()}</p><h2>준비 중인 기능</h2><p>이 기능은 Workflow 카드가 아닌 고유 원장과 기능 화면으로 제공됩니다.</p></div></div></section>}
    </section>
  </main>;
}
