...
프로젝트가 보이는 길은 둘뿐이다 — 그 프로젝트에 붙어 있거나, 그 조직에서 프로젝트를 관리할 자격이 있거나. 소유 조직은 열쇠가 아니다. 열쇠로 쓰면 배정되지 않은 팀원이 팀의 모든 프로젝트를 읽게 되어 `붙어야 보인다`는 규칙에 뒷문이 생긴다. 관리 자격을 남기는 이유는 하나다: 그것이 없으면 방금 만든 프로젝트를 만든 사람도 찾지 못해 아무도 배정할 수 없다.
