...
**한 번의 생성 명령으로 업무가 실재한다.** 본인 것이든 남에게 보낸 것이든 명령이 성공하면 그 업무가 그 자리에 선다. **다만 담당은 수락이 세운다** — 남에게 보낸 업무는 `open` 으로 서고 받는 사람이 수락해야 그 사람의 목록에 오른다. 본인 업무와 관리자 배정은 그대로 즉시 담당이 선다. 생성 명령은 `Idempotency-Key`를 필수로 받는다 — 한 번의 생성 의도에 키 하나를 만들어 재시도·연타 동안 같은 키를 다시 보내고, 새로 만드는 업무에는 새 키를 쓴다. 같은 키의 재전송은 첫 결과를 그대로 돌려주는 영수증이고(돌려주기 전에 지금 읽을 수 있는지 다시 검사한다), 같은 키에 내용이 다르면 충돌로 거절한다. 서버가 키를 대신 만들어 주는 길은 어느 표면에도 없다.

받은 것은 종류를 가리지 않고 **할일**로 모인다. **업무 요청이 여기 선다** — 받은 사람이 답해야 하는 질문 하나가 요청 하나에 붙는다(수락·거절·협의). 그 밖에 업무 결과 확인과 AX가 제안한 변경이 쌓이고, 예전에 만들어져 아직 답하지 않은 항목도 그대로 남아 계속 답할 수 있다. 관리자 배정은 즉시 담당이 서므로 여기 서지 않는다.

**요청**과 **배정**은 여전히 다르다.

- **요청**은 수평이다. 보낼 수 있는 사람인지만 보고 판단 권한은 묻지 않으므로, 배정 권한이 없는 구성원도 동료에게 보낼 수 있다. 후보는 들어올 문이 있고 · 본인이 아니고 · 조직 범위가 겹치는 사람이며, 화면에 뜬 후보와 명령이 같은 판정을 쓴다. 요청 행은 출처로 남아 업무가 `source_work_request_id`로 그것을 가리키고(완료 승인의 확인자가 거기서 나온다), 출처 상태는 `pending` — 업무는 섰고 **답을 기다린다**. 수락하면 `accepted` 이고 **같은 업무의 담당이 확정된다**(새 업무가 생기지 않는다). 거절·철회는 그 업무를 `cancelled` 로 닫되 상위 연결과 로그를 남기고, 다시 부탁하려면 **새 요청·새 Task** 를 보낸다(`supersedes_request_id` 가 둘을 잇는다). 하위 요청이면 발송 단계에서 `parent_task_id` 로 상위 아래에 선다. 참조자(cc)는 읽고 논의하되 판단하지 않는다. 요청자는 자기가 부탁한 업무와 **그 아래 전부**, 그리고 거기 연결된 자료를 읽는다 — 읽을 뿐이고 고치거나 시작하지는 못한다.
...
모든 업무는 `task_assignments` 행을 갖는다(self · request_effect · direct). `self`·`direct` 는 생성 시점에 `active` 이고 `request_effect` 는 `pending` 으로 선다. **한 업무의 활성 담당은 언제나 0 또는 1**이다 — 부분 unique 인덱스(`uq_task_assignments_active`)가 데이터베이스에서 지킨다. `내 업무`는 **active** 배정이 있는 것만 나열한다. 결과가 나오면 완료 보고를 올리고 요청자가 완료 인정 또는 보완 요청을 하며, 이것도 회차로 쌓인다.

업무는 `description` · `start_date` · `due_date`를 갖고 소유자가 `PATCH /api/tasks/{id}`로 고친다(`expected_version` 필수, 시작 ≤ 기한). 체크리스트, **깊이 제한 없는 하위 업무**, 참고 업무 연결이 그 위에 붙는다 — 화면은 중심 업무와 직속 하위 두 단계를 보이고 더 깊은 것은 그 업무로 들어가 읽는다. 같은 사람의 직접 작업을 무한히 겹치는 것만 막힌다. 모든 변경은 `TaskVersion`으로 남아 `활동·이력`에서 회차와 diff로 읽힌다.

**끝은 사람이 낸다.** 본인·배정 업무는 `complete` 가 최종 완료이고, 요청 업무는 담당자가 `completion-report` 로 **제출**하고 요청자가 **승인**해야 끝난다. 취소를 제외한 미완결 하위가 남아 있으면 최종 완료가 거부되고 **막는 하위를 이름으로** 알려 준다 — 하위가 요청 업무면 `done` 만으로는 완결이 아니고 요청자 승인까지여야 한다. 하위가 다 끝나도 자동 완료는 없다.

**수락된 요청 업무는 누구도 혼자 취소하지 못한다** — 요청자·담당자·관리자 모두 거부되고, 그 자리에서 취소로 가는 길은 **취소 제안 → 담당자 동의** 하나다. 수락 뒤에 조건을 바꾸는 것도 같은 제안–동의를 지난다. 완료한 업무는 다시 열 수 있고(상위가 이미 완료면 상위를 먼저 연다) 취소는 끝이다. 무응답과 기한 초과는 **표시만 바꾼다** — 자동 수락도 자동 완료도 없다.

밖으로 나가는 수행 상태는 **넷뿐이다**(`open` · `in_progress` · `done` · `cancelled`). 기다림은 상태가 아니라 파생 표시다: 응답 대기·담당 변경 대기·완료 확인 단계·대기 중인 제안·막는 하위·지연 일수를 서버가 `derived` 묶음으로 낸다 — 화면이 상태 문자열로 되짚지 않는다.
