...
import type { ActionCommand, ActionEditContract, ActionEditField } from './viewModels';
...
export function CommandConfirmationForm({ actionId, principalId, contract, commands, onCommand }: {
  ...
  return <form className="action-command-editor" aria-label="변경 내용 확인" onSubmit={event => { event.preventDefault(); if (confirm && !recovered.stale) void run(confirm.id); }}>
    ...
    <fieldset disabled={busy || Boolean(recovered.stale)} className="form-grid">
      {contract.fields.map(field => <div className="field" key={field.id}><label htmlFor={`${prefix}-${field.id}`}>{field.label}</label>{control(field)}</div>)}
    </fieldset>
    {error && <p role="alert">{error}</p>}
    <div className="action-task-buttons">
      {commands.map(command => <button key={command.id} className={`btn h36 ${command.tone === 'primary' ? 'primary' : 'ghost'}`} disabled={busy || (command.id === 'confirm' && Boolean(recovered.stale))} type={command.id === 'confirm' ? 'submit' : 'button'} onClick={command.id === 'confirm' ? undefined : () => void run(command.id)}>{command.label}</button>)}
      {recovered.restored && <button className="btn h36 ghost" disabled={busy} type="button" onClick={recovered.reset}>수정 취소</button>}
    </div>
  </form>;
}
