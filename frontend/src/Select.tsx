    ...
    <>
      {showSearch && (
        <div className="popover-head select-search">
          <input
            ...
            onKeyDown={onKeyDown}
            placeholder={searchPlaceholder ?? selectLabel.search}
            ref={searchRef}
            type="text"
            value={query}
          />
      ...
      {onToggleAll && (
        <div className="select-all">
          <button className="btn link" onClick={onToggleAll} type="button">
            {allSelected ? selectLabel.clearAll : selectLabel.selectAll}
          </button>
        </div>
      )}
      <div
      ...
      {matches.length === 0 && <Empty className="select-empty" onAction={() => setQuery("")} title={selectLabel.noMatch} variant="filter" />}
      {footerAction && (
...
export function Select({
  ...
  searchable?: boolean;
  searchPlaceholder?: string;
  /** 목록 바닥의 한 줄 행동 (`btn link`) — 예: "새 프로젝트로 추가" */
  footerAction?: SelectFooterAction;
  ...
  const text = current?.label ?? placeholder ?? selectLabel.placeholder;
...
export function MultiSelect({
  ...
  const text = picked.length > 0 ? picked.map((option) => option.label).join(", ") : (placeholder ?? selectLabel.placeholder);
  const describedBy = error || help ? messageId : undefined;
          ...
          return (
            ...
            <span className={disabled ? "select-trigger multi disabled" : "select-trigger multi"} ref={ref as React.Ref<HTMLSpanElement>}>
              <button {...triggerProps} className="select-open">
                {picked.length === 0 ? (
                  <span className="select-value placeholder">{placeholder ?? selectLabel.placeholder}</span>
                ) : (
                  chips.map((option) => (
                    <span className="badge ai" key={option.value}>
                      {option.label}
                    </span>
                  ))
                )}
              ...
              {picked.length > 0 && !disabled && (
                <button aria-label={selectLabel.clear} className="select-clear" onClick={() => onChange([])} type="button">
