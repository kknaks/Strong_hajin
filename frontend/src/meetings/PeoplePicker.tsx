...
import { CheckboxBox } from "../FormControls";
        ...
        <div aria-label="이름 찾기 결과" className="popover" role="listbox" style={{ left: 0, right: 0, width: "auto" }}>
          {hits.map((person) => (
            <button
              ...
              type="button"
            >
              <span className="avatar xs">{person.name.slice(0, 1)}</span>
              <span style={{ flex: 1, minWidth: 0, fontWeight: 700, color: "var(--text-primary)" }}>{person.name}</span>
          ...
          {hits.length > 0 && canAddGuest && guest && (
            <button
              ...
              type="button"
            >
              <span className="avatar xs">{text.slice(0, 1)}</span>
              <span style={{ flex: 1, minWidth: 0, fontWeight: 700, color: "var(--text-primary)" }}>{text}</span>
              <span className="t-meta" style={{ flex: "none", fontSize: 12 }}>
...
export function OrgDirectory({
  ...
  return (
    ...
    >
      <div className="meeting-scroll" style={{ maxHeight: 216, overflowY: "auto", padding: 8, borderRight: "1px solid var(--border-default)", background: "var(--surface-sunken)" }}>
        {units.map((unit) => {
          ...
          return (
            <button
              ...
              style={{
                ...
                color: on ? "var(--action)" : "var(--text-primary)",
    ...
    >
      {items.map((item) => (
        <button className="meeting-tag" key={item.key} onClick={item.onRemove} type="button">
          ...
          {item.note && <span style={{ color: "var(--text-tertiary)" }}>{item.note}</span>}
