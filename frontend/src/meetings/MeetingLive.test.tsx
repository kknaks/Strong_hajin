...
import type { MeetingAgenda, MeetingInfo, MeetingRecord, MeetingTodo } from "../viewModels";
...
/** 후속 업무 후보 하나 — 회의 중 배치가 내는 잠정 후보(provisional)와 종료 뒤 최종이 같은 모양이다. */
...
describe("SCR-106 「진행 중」 — 회의 스트림", () => {
  ...
  it("참여자는 구독으로 붙어 갱신되는 것을 다 받는다 — 보내는 것은 없다", async () => {
    ...
    // 두 탭은 그대로 서고 조작만 없다
    expect(screen.getByRole("button", { name: "메모" })).toBeTruthy();
  ...
  it("잠정 발화는 교체되고 확정 발화는 쌓인다", async () => {
    ...
    const liveBar = document.querySelector(".meeting-live-bar") as HTMLElement;
