...
function open(props: Partial<Parameters<typeof DatePicker>[0]> = {}) {
  ...
  render(<DatePicker label="종료일" onChange={onChange} value={VALUE} {...props} />);
...
describe("DatePicker", () => {
  it("닫혀 있을 때는 트리거뿐이다 — 화면에 달력이 없다", () => {
    render(<DatePicker label="종료일" onChange={vi.fn()} value={VALUE} />);
    const trigger = screen.getByRole("button", { name: "종료일 달력 열기" });
    expect(trigger.className).toContain("btn h30 ghost");
