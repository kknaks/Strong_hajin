...
import { emptyActionLabel, selectLabel, timeFieldLabel } from "../lib/labels";
...
describe("TimeField", () => {
  it("네이티브 input[type=time] 이 아니라 팝오버 목록이다", () => {
    render(<TimeField emptyActionLabel={emptyActionLabel.filter} labels={timeFieldLabel} selectLabels={selectLabel} label="회의 시각" onChange={vi.fn()} value="14:30" />);
  ...
  it("min·max 안의 30분 칸만 뜨고, 고르면 HH:MM 이 그대로 넘어온다", () => {
    ...
    render(<TimeField emptyActionLabel={emptyActionLabel.filter} labels={timeFieldLabel} selectLabels={selectLabel} label="회의 시각" max="20:00" min="09:00" onChange={onChange} value="" />);
  ...
  it("48칸이라 검색창이 저절로 붙는다", () => {
    render(<TimeField emptyActionLabel={emptyActionLabel.filter} labels={timeFieldLabel} selectLabels={selectLabel} label="회의 시각" onChange={vi.fn()} value="" />);
    fireEvent.click(screen.getByLabelText("회의 시각"));
  ...
  it("간격에 걸리지 않는 값도 목록에 남는다 — 열자마자 사라지지 않게", () => {
    render(<TimeField emptyActionLabel={emptyActionLabel.filter} labels={timeFieldLabel} selectLabels={selectLabel} label="회의 시각" onChange={vi.fn()} value="14:17" />);
    fireEvent.click(screen.getByLabelText("회의 시각"));
    expect(screen.getByRole("option", { name: "14:17" }).getAttribute("aria-selected")).toBe("true");
  });

  it("값이 없으면 placeholder 가 서고 비활성이면 열리지 않는다", () => {
    render(<TimeField emptyActionLabel={emptyActionLabel.filter} labels={timeFieldLabel} selectLabels={selectLabel} disabled label="회의 시각" onChange={vi.fn()} value="" />);
    const trigger = screen.getByLabelText("회의 시각");
...
describe("TimeRangeField", () => {
  it("한 쌍이 나란히 서고 각각 제 이름으로 읽힌다", () => {
    render(<TimeRangeField emptyActionLabel={emptyActionLabel.filter} endLabel={timeFieldLabel.end} labels={timeFieldLabel} selectLabels={selectLabel} startLabel={timeFieldLabel.start} end="15:00" label="회의 시간" onChange={vi.fn()} start="14:00" />);
  ...
  it("시작을 옮기면 종료가 같은 간격만큼 따라간다", () => {
    const onChange = vi.fn();
    render(<TimeRangeField emptyActionLabel={emptyActionLabel.filter} endLabel={timeFieldLabel.end} labels={timeFieldLabel} selectLabels={selectLabel} startLabel={timeFieldLabel.start} end="15:00" label="회의 시간" onChange={onChange} start="14:00" />);
  ...
  it("간격을 알 수 없으면 기본 60분으로 선다", () => {
    const onChange = vi.fn();
    render(<TimeRangeField emptyActionLabel={emptyActionLabel.filter} endLabel={timeFieldLabel.end} labels={timeFieldLabel} selectLabels={selectLabel} startLabel={timeFieldLabel.start} end="" label="회의 시간" onChange={onChange} start="" />);
    fireEvent.click(screen.getByLabelText("회의 시간 시작 시각"));
  ...
  it("defaultDuration 을 주면 그만큼 벌어진다", () => {
    const onChange = vi.fn();
    render(<TimeRangeField emptyActionLabel={emptyActionLabel.filter} endLabel={timeFieldLabel.end} labels={timeFieldLabel} selectLabels={selectLabel} startLabel={timeFieldLabel.start} defaultDuration={30} end="" label="회의 시간" onChange={onChange} start="" />);
    fireEvent.click(screen.getByLabelText("회의 시간 시작 시각"));
    fireEvent.click(screen.getByRole("option", { name: "09:00" }));
  ...
  it("종료가 시작보다 빠르면 한 줄로 말하고 종료 쪽 보더만 갈아입는다", () => {
    render(<TimeRangeField emptyActionLabel={emptyActionLabel.filter} endLabel={timeFieldLabel.end} labels={timeFieldLabel} selectLabels={selectLabel} startLabel={timeFieldLabel.start} end="13:00" label="회의 시간" onChange={vi.fn()} start="14:00" />);
  ...
  it("한쪽이 비어 있는 것은 잘못된 것이 아니다", () => {
    render(<TimeRangeField emptyActionLabel={emptyActionLabel.filter} endLabel={timeFieldLabel.end} labels={timeFieldLabel} selectLabels={selectLabel} startLabel={timeFieldLabel.start} end="" label="회의 시간" onChange={vi.fn()} start="14:00" />);
