...
import { emptyActionLabel, selectLabel } from "../lib/labels";
...
describe("Select", () => {
  it("네이티브 select 가 아니라 팝오버 위의 listbox 다", () => {
    render(<Select
            ...
            labels={selectLabel} label="담당자" onChange={vi.fn()} options={PEOPLE} value="lee" />);
  ...
  it("값이 없으면 placeholder 가 서고, 고르면 값만 넘어온다", () => {
    ...
    render(<Select
            emptyActionLabel={emptyActionLabel.filter}
            labels={selectLabel} label="담당자" onChange={onChange} options={PEOPLE} placeholder="담당자 고르기" value="" />);
  ...
  it("그룹은 값이 바뀔 때 한 번만 머리글을 낸다", () => {
    render(<Select
            emptyActionLabel={emptyActionLabel.filter}
            labels={selectLabel} label="담당자" onChange={vi.fn()} options={PEOPLE} value="" />);
    open("담당자");
  ...
  it("옵션이 8개를 넘으면 검색창이 저절로 붙고, 걸리는 것이 없으면 Empty(filter) 가 뜬다", () => {
    render(<Select
            emptyActionLabel={emptyActionLabel.filter}
            labels={selectLabel} label="항목" onChange={vi.fn()} options={MANY} value="" />);
    open("항목");
  ...
  it("↑↓ 로 옮기고 Enter 로 고른다 — 비활성 항목은 건너뛴다", () => {
    ...
    render(<Select
            emptyActionLabel={emptyActionLabel.filter}
            labels={selectLabel} label="담당자" onChange={onChange} options={PEOPLE} value="" />);
    open("담당자");
  ...
  it("footerAction 은 목록 바닥의 «글줄 속 단추» 다", () => {
    const onAction = vi.fn();
    render(<Select
            emptyActionLabel={emptyActionLabel.filter}
            labels={selectLabel} footerAction={{ label: "새 프로젝트로 추가", onAction }} label="프로젝트" onChange={vi.fn()} options={PEOPLE} value="" />);
    open("프로젝트");
    const action = screen.getByRole("button", { name: "새 프로젝트로 추가" });
    // 바퀴 9: 구 맨 클래스 `.btn link` 가 Button 의 inline 변형으로 흡수됐다 (바퀴 9)
    expect(action.className).toBe("scax-button scax-button--inline");
  ...
  it("에러는 FieldMessage 로 한 줄만 나가고 보더가 갈아입는다", () => {
    render(<Select
            emptyActionLabel={emptyActionLabel.filter}
            labels={selectLabel} error="담당자를 고르세요" help="같은 부서 사람만 보입니다" label="담당자" onChange={vi.fn()} options={PEOPLE} value="" />);
...
describe("MultiSelect", () => {
  it("고를 때마다 닫히지 않고, 값은 배열로 온다", () => {
    ...
    render(<MultiSelect
            emptyActionLabel={emptyActionLabel.filter}
            labels={selectLabel} label="참석자" onChange={onChange} options={PEOPLE} value={["kim"]} />);
    open("참석자");
  ...
  it("이미 고른 것을 다시 누르면 빠진다", () => {
    ...
    render(<MultiSelect
            emptyActionLabel={emptyActionLabel.filter}
            labels={selectLabel} label="참석자" onChange={onChange} options={PEOPLE} value={["kim", "lee"]} />);
    open("참석자");
    fireEvent.click(screen.getByRole("option", { name: /김서연/ }));
  ...
  it("칩은 두 개까지 서고 나머지는 +N 으로 접힌다", () => {
    render(<MultiSelect
            emptyActionLabel={emptyActionLabel.filter}
            labels={selectLabel} label="참석자" onChange={vi.fn()} options={PEOPLE} value={["kim", "lee", "park"]} />);
    const trigger = screen.getByLabelText("참석자");
    expect(trigger.querySelectorAll(".scax-badge").length).toBe(2);
  ...
  it("전체 선택은 고를 수 있는 것만 데려가고, 한 번 더 누르면 전부 풀린다", () => {
    ...
    const { rerender } = render(<MultiSelect
            emptyActionLabel={emptyActionLabel.filter}
            labels={selectLabel} label="참석자" onChange={onChange} options={PEOPLE} value={[]} />);
    open("참석자");
    fireEvent.click(screen.getByRole("button", { name: "전체 선택" }));
    expect(onChange).toHaveBeenCalledWith(["kim", "lee", "park"]);

    rerender(<MultiSelect
            emptyActionLabel={emptyActionLabel.filter}
            labels={selectLabel} label="참석자" onChange={onChange} options={PEOPLE} value={["kim", "lee", "park"]} />);
  ...
  it("옵션의 체크는 Checkbox 와 같은 네모이고, 목록은 다중 선택으로 읽힌다", () => {
    render(<MultiSelect
            emptyActionLabel={emptyActionLabel.filter}
            labels={selectLabel} label="참석자" onChange={vi.fn()} options={PEOPLE} value={["kim"]} />);
    open("참석자");
