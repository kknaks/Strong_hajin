...
describe("date field", () => {
  ...
  it("can use the dotted task-card presentation without changing the ISO boundary", () => {
    ...
    render(
      <DateField formatMonth={formatMonthLong} labels={datePickerLabel} today={seoulToday()} weekdayNames={weekdayNames}
        displaySeparator="."
        id="task-due"
        label="기한"
        onChange={onChange}
  ...
  it("값이 없으면 그 화면의 구분자로 자리표시를 낸다", () => {
    render(<DateField formatMonth={formatMonthLong} labels={datePickerLabel} today={seoulToday()} weekdayNames={weekdayNames} displaySeparator="." id="task-due" label="기한" onChange={vi.fn()} value="" />);
    expect(screen.getByRole("button", { name: "기한 달력 열기" }).textContent).toBe("YYYY.MM.DD");
  });

  it("칸을 누르면 우리 달력이 열리고 고른 날이 ISO 로 넘어간다", () => {
    const onChange = vi.fn();
    render(<DateField formatMonth={formatMonthLong} labels={datePickerLabel} today={seoulToday()} weekdayNames={weekdayNames} id="when" label="일시" onChange={onChange} value="2026-09-11" />);
  ...
  it("비활성이면 달력이 열리지 않는다", () => {
    render(<DateField formatMonth={formatMonthLong} labels={datePickerLabel} today={seoulToday()} weekdayNames={weekdayNames} disabled id="due" label="기한" onChange={vi.fn()} value="2026-09-30" />);
    const field = screen.getByRole("button", { name: "기한 달력 열기" });
