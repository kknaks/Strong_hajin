 ...
 * 아니다.
 *
 * **바퀴 3a — 껍데기만 새 DS 로 갈았다.** 패널이 `.popover` 대신 `.scax-popover` 를 입는다.
 * 동작(포털 · 위아래 뒤집기 · 남은 자리만큼 자라기 · Esc 겹 스택 · 포커스 되돌리기)과 props
 * (`trigger` 렌더프롭 · `children(close)` · `label` · `width`)는 **그대로 뒀다**(D-4).
 * 핸드오프 `scax-ui.jsx` 의 `Popover({open,onClose,placement})` 는 트리거 밑에 절대 위치로 붙는
 * 단순한 층이라, 그대로 옮기면 DS-18 이 고쳐 둔 «조상 overflow 에 잘림» 이 되살아난다.
 * 목록 한 줄(`.popover-item`)과 머리·게스트 줄은 아직 구 클래스다 — 그 자리를 쓰는 `Select`·
 * `PeoplePicker` 가 폼 부품이라 바퀴 3b 에서 `.scax-popover__option` 과 함께 간다.
 */
    ...
    <span className="popover-root" ref={rootRef}>
      ...
      {open &&
        createPortal(
          <div
            aria-label={label}
            className={placement.above ? "scax-popover scax-popover--above" : "scax-popover"}
            id={panelId}
            ref={panelRef}
            role="group"
            /*
             ...
             *
             * 바퀴 3a: 껍데기(테두리·라운드·바탕·그림자·안쪽 여백)는 새 DS 의 `.scax-popover` 가 갖고,
             * «기하» 는 예전처럼 여기서 준다. 새 DS 규칙은 `position:absolute` 로 트리거 밑에 붙는 팝오버를
             * 전제하는데(`--below-start`) 우리는 포털로 body 에 띄우고 좌표를 직접 재기 때문이다.
             * 목록 배치와 «패널 안에서 스크롤» 도 새 DS 규칙에 없어서 함께 여기로 옮겼다 — 이것을 잃으면
             * 내용이 max-height 를 넘겼을 때 잘린 채 스크롤이 안 된다.
             */
            style={{
              position: "fixed",
              top: placement.top,
