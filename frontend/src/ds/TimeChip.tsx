 ...
 *
 * 바퀴 3b: 껍데기가 새 DS 의 `.scax-time-chip`(`styles/workspace.css`) 으로 갔다 — `DS-gaps` G-12 가
 * 「CSS 는 완전하고 우리 부품이 하는 일은 로직이라 우리 것」이라고 판정한 그대로다(㉮ · 사다리 ③).
 */
export function TimeChip({ label, onClick, title }: { label: string; onClick?: () => void; title?: string }) {
  if (!onClick) {
    return (
      <span className="scax-time-chip" title={title}>
        {label}
      </span>
    );
  }
  return (
    <button className="scax-time-chip" onClick={onClick} title={title} type="button">
      {label}
    </button>
  );
}
