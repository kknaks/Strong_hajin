...
describe("AssistantCharacter", () => {
  ...
  it("keeps a static poster available until the production image loads and after an image error", () => {
    ...
    expect(container.querySelector(".assistant-character-fallback")).toBeTruthy();

    fireEvent.load(image);
    expect(container.querySelector(".assistant-character-fallback")).toBeNull();
    rerender(<AssistantCharacter characterKey="red-panda" state={idle} />);
    expect(container.querySelector(".assistant-character-fallback")?.textContent).toBe("레");
  ...
  it("opens from the character and only prefills from the separate DOM prompt", () => {
    ...
    const launcher = screen.getByRole("button", { name: "AX" }).parentElement!;
  ...
  it("pauses decorative motion while the document is hidden", () => {
    ...
    expect(screen.getByRole("button", { name: "AX" }).parentElement?.classList.contains("motion-paused")).toBe(true);
  });
});
