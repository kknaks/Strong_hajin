...
import { ApiError, getOrganizationMemberHistory, type MemberHistoryAxis, type MemberHistoryEntry } from "../../lib/api";
import { Empty } from "../../ds/Empty";
import { Popover } from "../../ds/Popover";
import { Skeleton } from "../../ds/Skeleton";
import { formatPeriod, orgScreen } from "../../lib/labels";
  ...
  return (
    <div>
      {view.rows.map((row, index) => (
        <div className="org-history-row" key={`${row.value ?? ""}-${row.valid_from}-${index}`}>
          <div className="org-history-row__top">
            <span className="org-history-row__value">
              {row.value ?? "—"}
              {row.unit_name && row.unit_name !== row.value ? ` · ${row.unit_name}` : ""}
            </span>
            <span className="org-history-row__when">{formatPeriod(row.valid_from, row.valid_until)}</span>
          </div>
          <div className="org-history-row__bottom">
            <span className="org-history-row__reason">{row.reason ?? orgScreen.historyNoReason}</span>
            {row.actor && <span className="org-history-row__actor">{row.actor}</span>}
          </div>
        </div>
      ))}
    </div>
...
export function AxisHistoryButton({
  ...
  if (!axis) {
    return (
      <Button variant="text" size="sm" disabled title={orgScreen.historyNotAnAxis} type="button">
        {orgScreen.history}
      </Button>
    );
  }
  return (
    ...
    >
      {() => (
        <>
          <div className="popover-head org-history-head">
            <b>{title}</b>
          </div>
