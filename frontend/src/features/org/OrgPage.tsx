...
import { ConfirmModal, Toast } from "../../ds/Modal";
import { orgScreen, personName } from "../../lib/labels";
import { AccessDrawer } from "./AccessDrawer";
import { ChangeLogPanel } from "./ChangeLogPanel";
import { MemberAxesPanel, grantText, type DetailView } from "./MemberAxesPanel";
...
export function OrgPage({ personaId, onError }: OrgPageProps) {
  ...
  return (
    <section className="page-surface">
      {/* 바퀴 8-B: 페이지 h1 「조직」을 걷어냈다 — 셸 머리(.scax-page-header > h1)가 같은 글자를
          이미 이고 있었다(바퀴 5a J-2 와 같은 처리). 권한 배지는 값이라 이 얇은 줄이 받는다.
      ...
      {toast && <Toast
          closeLabel="알림 지우기" message={toast.message} onClose={() => setToast(null)} tone={toast.tone} />}
    </section>
  );
}
