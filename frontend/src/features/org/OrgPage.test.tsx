...
describe("OrgPage — 조직 화면 v3", () => {
  ...
  it("직원에게는 변경 기록도 권한 변경도 없고, 남의 권한은 자격이 없다고 말한다", async () => {
    ...
    // 바퀴 8-B: 페이지 h1 「조직」은 셸 머리와 글자까지 같은 두 번째 줄이라 걷어냈다. 배지는 그대로
    // 있고 자리만 `.screens-b-lead` 로 옮겼다 — 「조직 tree」 패널의 같은 낱말과 구별하려고 자리를 쥔다.
    const head = (await screen.findByText("읽기 전용", { selector: ".screens-b-lead .scax-badge" })).closest(
      ".screens-b-lead",
    ) as HTMLElement;
...
describe("OrgPage — 변경 기록", () => {
  ...
  it("n건이면 핸드오프의 표로 그리고 축마다 배지를 붙인다", async () => {
    ...
    expect(within(rows[0]).getByText("권한").className).toContain("scax-badge--accent");
    expect(within(rows[1]).getByText("소속").className).toContain("scax-badge--neutral");
  });
