...
describe("task checklist", () => {
  ...
  it("lists the steps in order with progress, and marks the finished ones", async () => {
    ...
    expect(Array.from(section.querySelectorAll(".scax-checklist__row .checkbox-label")).map((node) => node.textContent)).toEqual([
      "자료 모으기",
      "초안 쓰기",
      "검토 요청",
    ]);
...
describe("문서 상태", () => {
  ...
  it("says a scan could not be read instead of showing it as an empty success", async () => {
    ...
    expect(container.querySelector(".scax-badge--danger")).toBeNull(); // it is not a failure of ours
