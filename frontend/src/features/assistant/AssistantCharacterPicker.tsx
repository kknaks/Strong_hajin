...
export function AssistantCharacterPicker({
  ...
  return (
    <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && !busy && onClose()}>
      <section aria-label="내 AX 캐릭터" aria-modal="true" className="modal scax-character-picker" role="dialog">
        ...
        {error && <p className="scax-character-picker__error" role="alert">{error}</p>}
        <div aria-label="AX 캐릭터 목록" className="scax-character-picker__grid" role="radiogroup">
          {assistantCharacterCatalog.map((asset) => (
            <button
              aria-checked={resolved.key === asset.key}
              aria-label={`${asset.name} 선택`}
              className="scax-character-picker__option"
              disabled={busy}
              key={asset.key}
