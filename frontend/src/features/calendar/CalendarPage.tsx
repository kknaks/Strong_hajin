...
import { getMyWork, getTasks, transitionDirectTask, updateTask } from "../../lib/api";
import { personName } from "../../lib/labels";
import type { DirectTask, Persona, TaskPatch } from "../../lib/viewModels";
import { TaskDetailDrawer, type TaskAction } from "../work/WorkModals";
import { TaskCalendar } from "../work/WorkViews";
...
export function CalendarPage({
  ...
  return (
    /* 바퀴 8-C: `calendar-screen` 을 뗐다 — 8-B 가 `.calendar-*` 를 이 화면 안에 가두려고 붙인
       울타리인데, 그 가둠을 풀면서 이 이름을 짚는 규칙이 하나도 남지 않았다. */
    <section className="page-surface">
      {/* 바퀴 8-B: 페이지 h1 「캘린더」를 걷어냈다 — 바퀴 2 가 세운 셸 머리(.scax-page-header > h1)가
          같은 글자를 이미 이고 있어서 같은 제목이 두 줄이었다. 바퀴 5a 가 업무 화면에서 한 것과 같다(J-2).
          설명 문단은 값이라 남기고, 새 DS 의 얇은 머리줄이 받는다. */}
      <div className="screens-b-lead">
        {/* 회의는 이제 좌측 메뉴 「회의 목록」 아래 전체 화면 둘이 맡는다 — 이 화면은 업무만 낸다. */}
        <p className="screens-b-lead__text">
          업무는 계획한 날짜로만 표시합니다. 시작일과 기한이 모두 있으면 그 기간, 하나만 있으면 그 날 하루, 없으면 캘린더에 두지 않습니다.
        </p>
      </div>
