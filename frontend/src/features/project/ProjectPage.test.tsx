...
describe("프로젝트", () => {
  ...
  /* 바퀴 8-C §1: 셸 머리(AppHeader)가 이미 「프로젝트」를 이고 있는데 이 화면이 자기 머리로 또 그렸다.
     걷어낸 뒤에 그 사실을 지키는 검사가 없으면 다음 바퀴가 되돌려도 아무도 모른다. */
  it("제목을 스스로 또 그리지 않는다 — 머리는 셸 것 하나다", async () => {
    ...
    // 설명 문단은 값이라 남는다 — 제목만 걷어냈지 내용을 버린 것이 아니다
    expect(screen.getByText(/부서를 가로질러 묶이는 일입니다/)).toBeTruthy();
  });
