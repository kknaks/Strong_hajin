...
import { emptyActionLabel, formatDate, personName, selectLabel, taskStateLabel } from "../../lib/labels";
import { Select } from "../../ds/Select";
import type { Persona, Project, ProjectDetail, ProjectParticipation } from "../../lib/viewModels";
...
export function ProjectPage({ personaId, onError }: { personaId: string; onError: (message: string | null) => void }) {
  ...
  return (
    <section aria-label="프로젝트" className="project-page">
      {/* 바퀴 8-C: 페이지 제목 「프로젝트」를 걷어냈다 — 셸 머리(.scax-page-header > h1)가 같은 글자를
          이미 이고 있었다(바퀴 5a J-2 · 8-B 가 세 화면에 한 것과 같은 처리). 설명 문단은 값이라 남기고,
          8-B 가 세워 둔 얇은 줄(`.screens-b-lead`)이 받는다. 이 화면에는 머리 행동이 없다. */}
      ...
      <div className="project-layout">
        <aside className="project-list">
          <div className="project-list-head">
            ...
            <Button size="sm" onClick={() => setOpening((current) => !current)} type="button">
              {opening ? "취소" : "새 프로젝트"}
            </Button>
          </div>
          {opening && (
            <div className="project-new">
              ...
              <Button variant="solid" tone="primary" size="sm" disabled={busy || !name.trim()} onClick={() => void open()} type="button">
                열기
              </Button>
            </div>
          )}
        ...
        {selected === null ? (
          ...
          <div className="project-detail" data-project-id={selected.project_id}>
            ...
            <section aria-label="현재 참여자">
              ...
              <ul className="project-members">
                {selected.members.map((row) => (
                  <li data-member-id={row.member_id} key={row.member_id}>
                    <Badge tone={row.assignment_kind === "lead" ? "accent" : "neutral"}>{row.assignment_kind === "lead" ? "담당" : "참여"}</Badge>
                    <span>{personName(row.display_name)}</span>
                    {row.valid_until && <span className="t-meta">~ {formatDate(row.valid_until.slice(0, 10))}</span>}
                    {selected.may_manage && (
                      <Button
                        ariaLabel={`${personName(row.display_name)} 참여 종료`}
                        variant="text"
                        size="sm"
                        disabled={busy}
                        onClick={() => {
              ...
              {releasing && (
                <div className="project-join">
                  ...
                  <Button size="sm" disabled={busy} onClick={() => void release(releasing.memberId)} type="button">
                    종료 기록
                  </Button>
            ...
            labels={selectLabel}
                  ...
                  value={joining}
                />
                <Button size="sm" disabled={busy || !joining} onClick={() => void join(joining, "member")} type="button">
                  참여로 붙이기
                </Button>
                <Button size="sm" disabled={busy || !joining} onClick={() => void join(joining, "lead")} type="button">
                  담당으로 붙이기
                </Button>
              </div>
              )}
