...
import type { ActionEditContract, ActionEditField, ActionEditOption, ActionItem, ActionMaterialDraft } from "../../lib/viewModels";
      ...
      return (
        <div className="action-task-list" id={id}>
          {items.length > 0 && <ul>{items.map((item, index) => (
            <li key={`${item}-${index}`}>
              <span>{item}</span>
              <Button variant="text" size="sm" aria-label={`${item} 빼기`} disabled={disabled} onClick={() => onChange(replace(draft, field.id, items.filter((_, position) => position !== index)))}
                type="button"
              >
                빼기
              </Button>
          ...
          <div className="action-task-add-row">
            <input
              ...
              value={pending}
            />
            <Button size="sm" disabled={disabled || !pending.trim()} onClick={() => {
...
export function ActionTaskCard({
  ...
  // 톤은 DS Badge 의 것이다 (바퀴 7). 구 `.action-task-state-badge` 네 갈래를 그대로 옮겼다:
  // draft/rejected→neutral · task/approved→info · requested→accent.
  // 구 「요청」은 주황(#ff5d00)이었는데 새 Badge 에 경고 톤이 없어 accent 로 옮겼다 — 보고 대상.
  const statusBadge: { label: string; tone: BadgeTone } = action.state === "pending"
    ? pendingAssignment ? { label: "요청", tone: "accent" } : { label: "초안", tone: "neutral" }
    : awaitingAssignee
      ? { label: "요청", tone: "accent" }
      : requested && action.result?.status === "cancelled"
        ? { label: "취소됨", tone: "neutral" }
        : requested && action.result?.status === "declined"
          ? { label: "거절됨", tone: "neutral" }
          : requested && action.result?.status === "active"
            ? { label: "수락됨", tone: "info" }
            : taskId
              ? { label: "내 업무", tone: "info" }
    ...
    <>
      {pendingAssignment && requestTarget?.value && (
        <div aria-label="요청 대상" className="action-task-request-target">
          <Avatar name={requestTargetName} size="sm" />
      ...
      <div className="action-task-actions">
        ...
        {action.state === "pending" && contract && editing && (
          <>
            <Button variant="text" size="sm" className="action-task-cancel" disabled={busy || uploadingMaterial} onClick={() => {
              recovered.reset();
              setError(null);
        ...
        {(action.commands ?? []).filter((command) => !["confirm", "reject"].includes(command.id)).map((command) => (
          <Button
            ...
            size="sm"
            tone={command.tone === "danger" ? "danger" : "neutral"}
            type="button"
            variant={command.tone === "danger" ? "solid" : "text"}
          >
            {command.label}
          </Button>
        ))}
        {taskId && onOpenTask && (
          <Button size="sm" onClick={() => onOpenTask(taskId)} type="button">
        ...
        <ul className="action-task-attachment-list">
          ...
          {materials.map((material) => (
            <li data-attachment-kind={material.source_kind} key={material.material_draft_id}>
              <span aria-hidden><Icon name={material.source_kind === "file" ? "document" : "link"} size={16} /></span>
              <span>
                {material.name}
          ...
          {transfers.map((transfer) => (
            <li data-attachment-kind="file" data-upload-state={transfer.state} key={transfer.id}>
              <span aria-hidden><Icon name="document" size={16} /></span>
    ...
    <div className="modal-backdrop attachment-picker-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section aria-label={referencesOnly ? "참고 업무 연결" : "업무나 자료 첨부"} aria-modal="true" className="modal attachment-picker-modal" role="dialog">
        ...
        <footer className="attachment-picker-foot">
          ...
          <div><Button variant="text" onClick={onClose} type="button">취소</Button><Button variant="solid" tone="primary" disabled={pending.length === 0} onClick={() => onConfirm(pending)} type="button">첨부</Button></div>
        </footer>
      </section>
    </div>
  );
...
/**
 * 첨부 찾기 판의 빈 상태 넷(입력 전 · 검색 중 · 실패 · 결과 없음).
 *
 * 바퀴 7 에서 손으로 짠 `.attachment-picker-empty` 를 DS `Empty` 로 갈았다.
 * 아이콘이 `search` 에서 `Empty` 가 정한 `inbox` 로 바뀐다 — `Empty` 는 아이콘을 prop 으로
 * 받지 않고, 공용 부품의 시그니처는 이 바퀴가 넓히지 않는다(P-3). 보고 대상이다.
 */
function AttachmentSearchEmpty({ title, detail }: { title: string; detail: string }) {
  return <Empty description={detail} title={title} />;
}
...
function StaleTaskDraft({
  ...
  return (
    <div aria-label="저장 초안 충돌" className="action-task-stale" role="status">
      ...
      <div className="action-task-actions">
        <Button size="sm" onClick={onRestart} type="button">최신안으로 다시 시작</Button>
        <Button variant="text" size="sm" onClick={onContinue} type="button">내 초안으로 다시 편집</Button>
      </div>
    </div>
  );
}
