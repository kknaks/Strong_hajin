...
export function ActionProgressBatchCard({
  ...
  return (
    <section className="scax-actioncard action-progress-batch-card" data-action-id={action.action_id} data-state={action.state}>
      <Badge tone="accent">{actionKicker(action)}{action.state === "pending" ? " · 승인 필요" : ""}</Badge>
        ...
        <div className="action-progress-batch-editor" aria-label="업무 진행 수정">
          {operations.map((operation, index) => (
            <fieldset className="action-progress-batch-row" key={operation.effect_id ?? `${operation.task_id}:${index}`}>
              ...
              <Button variant="text" size="sm" disabled={busy} onClick={() => setOperations((current) => current.filter((_, itemIndex) => itemIndex !== index))}
                type="button"
              >
                제외
              </Button>
      ...
      {action.state === "pending" && contract && (
        <div className="action-progress-batch-actions">
          {editing ? (
            <>
              <Button variant="text" size="sm" disabled={busy} onClick={() => { setOperations(base); setEditing(false); setError(null); }} type="button">취소</Button>
              <Button variant="solid" tone="primary" size="sm" disabled={busy || operations.length === 0 || operations.some((item) => item.kind === "progress.note" && !item.summary?.trim())}
            ...
            <>
              <Button variant="text" size="sm" disabled={busy} onClick={() => setEditing(true)} type="button">수정</Button>
              {confirm && (
                <Button variant="solid" tone="primary" size="sm" disabled={busy} onClick={() => void run(confirm.id, { base_submission_version: contract.base_submission_version })}
                  type="button"
                >
                  {busy ? "반영 중…" : confirm.label}
                </Button>
              )}
              {reject && <Button variant="text" size="sm" disabled={busy} onClick={() => void run(reject.id)} type="button">{reject.label}</Button>}
            </>
          )}
        </div>
      )}
