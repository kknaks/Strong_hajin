...
- **채팅의 탐색 경로는 실제로 걸어간 발자국이다.** 위임 turn이 graph tool을 부르면 그때 실제로 반환된 node·edge만 `conversation_graph_receipts`에 순서대로 남고, 대화 상세가 그것을 그대로 투영한다. 모델이 추측한 관계나 부르지 않은 조회는 없으며, 다른 대화로 갔다 돌아와도 PostgreSQL에서 복구된다.
...
| (MATERIAL projection, SPEC-006) | `material_extractions`, `material_blocks`, `material_chunks` | Attachment 버전(`integrity_ref`)별 추출 lifecycle(queued/running/completed/failed/unsupported, 실패 이유 코드)과 bounded chunk. 원본은 Attachment이고 이들은 파생 projection이다. **문서는 자기 모양대로 남는다**: DOCX/XLSX/PPTX/PDF는 parser가 만든 block(문단·표·시트 행·슬라이드·쪽) 단위로 `material_blocks`에 저장되고 검색용 chunk는 거기서 파생되며 `block_id`로 출처를 가리킨다 — 재색인에 원본 bytes가 다시 필요하지 않다. parser를 올리면 이전 extraction은 `superseded_at`로 남고 새 row가 생긴다(`(attachment, integrity_ref, parser_version)` unique); 검색은 현재 것만 읽는다. 텍스트 층이 없는 스캔은 `needs_ocr`로, 완전 삭제된 자료는 `purged`로 말하고 빈 성공으로 가장하지 않는다. `purge_attachment`는 bytes·block·chunk를 지우고 AX 근거 행은 남긴 채 excerpt만 비운다 — 무엇을 근거로 답했는지는 남고 그 내용은 사라진다. `GET /api/tasks/{id}/materials/search`·MCP `task_material_search`가 active assignment·live binding을 재검증한 뒤 lexical retrieval(port 교체 가능)로 excerpt·파일명·integrity·origin만 반환한다 |
