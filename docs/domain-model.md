...
- **kind별 MCP 판단 tool은 deprecated다.** `work_request_accept|negotiate|reject|resubmit`, `task_assignment_inbox|accept|decline`은 별도 domain transition은 아니지만(같은 application operation을 부른다) canonical envelope를 우회하는 두 번째 판단 표면이다: `allowed_commands` policy와 payload-aware receipt를 거치지 않고 `action_item_id`가 아닌 resource id로 동작한다. 기존 caller 호환을 위해 남겨 두되 description과 server instructions가 `action_item_*`로 안내하고, 새 판단은 canonical 경로만 쓴다.
