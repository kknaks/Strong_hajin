...
- **판단은 자기가 본 근거를 동결한다.** accept·reject·negotiate는 결정 시점 Submission의 evidence manifest를 canonical 형태(정렬된 `attachment_id`·`evidence_role`·`fixed_snapshot_ref`)로 계산해 `ReviewDecision.conditions`의 예약 key `evidence_hash`·`evidence_manifest`에 남긴다. 기존 조건(`changes`·`note` 등)은 그대로 보존한다. manifest identity는 Evidence row id가 아니라 attachment와 그 무결성이므로 row 생성 순서와 무관하게 같은 집합은 같은 hash이고, 빈 근거도 고유한 hash를 갖는다. 판단이 끝난 회차에는 근거를 더할 수 없으므로 그 회차의 현재 manifest는 동결된 hash와 영원히 같고, 근거는 다음 회차에서만 자란다. WorkRequest timeline과 ActionItem 상세가 회차별 현재 manifest·hash와 각 결정이 동결한 hash를 권한 범위 안에서 함께 보여 준다.
...
- **근거 채택이 판단 version을 소비한다.** 채택이 `work_requests.version`을 올리므로, 이미 내려진 command를 응답 유실 뒤 재전송하면 payload-aware receipt 대신 stale 422가 돌아오고(`is_replay`가 `version == expected+1`을 요구한다), 위임 turn의 pending wrapper도 제3자의 파일 업로드 하나로 승인 불가가 된다. 어느 쪽도 중복 effect를 만들지 않으므로 안전 방향의 축소이지만, receipt 창과 stale-wrapper 범위가 그만큼 좁아졌다. 근거 변경을 판단 version과 분리하거나 replay 판정에서 evidence 유래 bump를 구분하는 것은 별도 설계 결정이다.
...
- **위임 turn은 판단을 준비만 하고, 사람이 만든다.** `AX_MCP_CAUSATION_ID`가 있는 chat turn에서 `action_item_command`는 effect를 실행하지 않고 `action_item.command` 타입의 gated AX Action을 제안한다. 저장되는 payload는 server가 정규화한 canonical form이다: target `action_item_id`·`command`·필수 `expected_version`, 그리고 그 kind가 실제로 받는 `reason`·`changes`만 남는다. 사람이 그 카드를 REST/UI로 승인할 때만 canonical `ActionCenterApplication.execute`가 target에 정확히 한 번 적용한다.
  ...
  - **Residual.** target이 확인 대기 중에 움직이거나 승인자의 권한이 회수되면 승인이 422로 거절되고 wrapper는 pending으로 남는다(effect 0). 사람이 거절해 치우는 것 외의 정리 UX는 아직 없다. `platform/actions`와 `platform/action_center`는 서로를 참조하며 함수 안 지연 import로 순환을 푼다.
